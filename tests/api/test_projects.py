...
def test_create_project_retourne_201(client):
    ...
    with patch("app.api.routes.projects.services.create_project", return_value=fake):
...
def test_list_projects_retourne_200(client):
    with patch("app.api.routes.projects.services.list_projects", return_value=[]):
...
def test_get_project_retourne_404_si_absent(client):
    with patch("app.api.routes.projects.services.get_project", return_value=None):
...
def test_get_project_retourne_200_si_trouve(client):
    ...
    with patch("app.api.routes.projects.services.get_project", return_value=fake):
        response = client.get(f"/projects/{PROJECT_ID}")
    assert response.status_code == 200
...
def test_delete_project_retourne_204(client):
    with patch("app.api.routes.projects.services.delete_project", return_value=True):
        response = client.delete(f"/projects/{PROJECT_ID}")
    assert response.status_code == 204


def test_delete_project_retourne_404_si_absent(client):
    with patch("app.api.routes.projects.services.delete_project", return_value=False):
        response = client.delete(f"/projects/{PROJECT_ID}")
    assert response.status_code == 404
...
DIMENSIONING_PARAMS = "panel_peak_power_wp=400&battery_capacity_wh=200&battery_dod=0.8"


def test_dimensioning_retourne_200(client):
    with patch("app.api.routes.projects.services.get_dimensioning", return_value=FAKE_DIMENSIONING):
...
def test_dimensioning_retourne_404_si_projet_absent(client):
    with patch("app.api.routes.projects.services.get_dimensioning", return_value=None):
        response = client.get(f"/projects/{PROJECT_ID}/dimensioning?{DIMENSIONING_PARAMS}")
    assert response.status_code == 404
...
def test_dimensioning_retourne_409_si_irradiance_absente(client):
    with patch(
        "app.api.routes.projects.services.get_dimensioning",
...
def test_dimensioning_valide_battery_dod(client):
    response = client.get(f"/projects/{PROJECT_ID}/dimensioning?panel_peak_power_wp=400&battery_capacity_wh=200&battery_dod=1.5")
    assert response.status_code == 422
